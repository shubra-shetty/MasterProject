 ...
 //       resultsList = (TextView) findViewById(R.id.results);
  ...
  /*  private class MessageEndpointTask 
    ...
    private class ListOfPlacesAsyncRetriever extends AsyncTask<Void, Void, CollectionResponseMyCalculations> {
      ...
      float lat = (float) gps.getLatitude();
      float lon = (float) gps.getLongitude();
      GeoPt myLoc = new GeoPt();
      myLoc.setLatitude(lat);
      myLoc.setLongitude(lon);
      myCalc.setLocation(myLoc);
      myCalc.setAddress("Buckley St Santa Clara");
      myCalc.setPlaceId(friend1);	//("Shubra");
      myCalc.setName(friend2);	//("sjsu");        
...
// 		myCalc.setKey(k);
     	...
     	System.out.println("Meet-n-Eat:  " + myCalc.getPlaceId());
      
       try {      
    //    	result = endpoint.listMyCalculations().execute();
    	   result = new CollectionResponseMyCalculations();
 /*     	  System.out.println("Meet-n-Eat: listMyCalculation() CALL.");
          ...
          myCalc = myCalcendpoint.updateMyCalculations(myCalc).execute();	//Update myLoc object with midPoint - runs on server
        } catch (IOException e) {
       ...
       System.out.println("Meet-n-Eat: updateMyCalculation() DONE.");  
  ...
  /* 		   try {
	...
	class LoadPlaces extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... args) {
			...
			// Getting intent data
						Intent i = getIntent();
						 Bundle choices = i.getExtras();
						
						// Users current choices
						String f = choices.getString("rating");
	...
	//					gps.getLongitude(), radius, types);
				...
				//System.out.println("midpts  .... "+ mlat + mlong);
				//System.out.println("midpts  .... "+ mlat + mlong);
				nearPlaces = googlePlaces.search(midpt_lat, midpt_lon, radius, types,rating);
