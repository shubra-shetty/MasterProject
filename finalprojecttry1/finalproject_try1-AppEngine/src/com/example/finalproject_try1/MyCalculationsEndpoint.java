...
//			System.out.println("Meet-n-Eat: Generated key for string : " + KeyFactory.keyToString(k1));		
			...
			if(doThreePointMidpt) {				
				...
				if(contains) {
					System.out.println("Meet-n-Eat: Contains TRUE.");
					updateMidPoint_3Points(mycalculations, frndCalc, frndCalc_2);
				}
				else
					throw new EntityNotFoundException("Object does not exist");
			}
...
private void updateMidPoint_3Points(MyCalculations userCalc, MyCalculations frndCalc1, MyCalculations frndCalc2) {
	double inlat1 = (double) userCalc.getLocation().getLatitude(); // mycalculations.getMidpointLat();
	double inlon1 = (double) userCalc.getLocation().getLongitude();
	double inlat2 = (double) frndCalc1.getLocation().getLatitude();	// 37.4075209; // N
	double inlon2 = (double) frndCalc1.getLocation().getLongitude();	//-121.9376168; // W
	double inlat3 = (double) frndCalc2.getLocation().getLatitude();	// 37.4075209; // N
	double inlon3 = (double) frndCalc2.getLocation().getLongitude();	//-121.9376168; // W
	
 //Math for midpoint b/w user and frnd1
	double dLon = Math.toRadians(inlon2 - inlon1);

 	// convert to radians
 	double lat1 = Math.toRadians(inlat1);
 	double lat2 = Math.toRadians(inlat2);
 	double lon1 = Math.toRadians(inlon1);

 	double Bx = Math.cos(lat2) * Math.cos(dLon);
 	double By = Math.cos(lat2) * Math.sin(dLon);
 	double tmpMidptLat1 = Math.atan2(
 			Math.sin(lat1) + Math.sin(lat2),
 			Math.sqrt((Math.cos(lat1) + Bx) * (Math.cos(lat1) + Bx) + By
 					* By));
 	double tmpMidptLon1 = lon1 + Math.atan2(By, Math.cos(lat1) + Bx);
 	tmpMidptLat1 = Math.toDegrees(tmpMidptLat1);
 	tmpMidptLon1 = Math.toDegrees(tmpMidptLon1);
 	
 	//Math for midpoint b/w user and frnd2
 		dLon = Math.toRadians(inlon3 - inlon1);

 	 	// convert to radians
 	 	lat1 = Math.toRadians(inlat1);
 	 	lat2 = Math.toRadians(inlat3);
 	 	lon1 = Math.toRadians(inlon1);

 	 	Bx = Math.cos(lat2) * Math.cos(dLon);
 	 	By = Math.cos(lat2) * Math.sin(dLon);
 	 	double tmpMidptLat2 = Math.atan2(
 	 			Math.sin(lat1) + Math.sin(lat2),
 	 			Math.sqrt((Math.cos(lat1) + Bx) * (Math.cos(lat1) + Bx) + By
 	 					* By));
 	 	double tmpMidptLon2 = lon1 + Math.atan2(By, Math.cos(lat1) + Bx);	
 	 	tmpMidptLat2 = Math.toDegrees(tmpMidptLat2);
 	 	tmpMidptLon2 = Math.toDegrees(tmpMidptLon2);
 	
 	 //Math for midpoint b/w the 2 temporary midpoints
 		dLon = Math.toRadians(tmpMidptLon2 - tmpMidptLon1);

 	 	// convert to radians
 	 	lat1 = Math.toRadians(tmpMidptLat1);
 	 	lat2 = Math.toRadians(tmpMidptLat2);
 	 	lon1 = Math.toRadians(tmpMidptLon1);
